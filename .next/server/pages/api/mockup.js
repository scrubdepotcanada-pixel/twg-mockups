...
Be accurate. Use real info. If you can't find something, infer reasonably. Return ${p?"4":"3"} signature_items and ${p?"9":"6"} all_items.${p?" Write richer, more compelling descriptions. Make the tagline creative and memorable. Write a longer, more engaging about_paragraph.":""} Do NOT include any HTML tags, citations, or source references in the JSON values — plain text only.`,g=`Research this business:

Business: ${n}
Address: ${o}${u?`
Business Type: ${u}`:""}${i?`
Context: ${i}`:""}`;if(s=l?await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${h}`},body:JSON.stringify({model:"gpt-5.5",messages:[{role:"system",content:m},{role:"user",content:g}],response_format:{type:"json_object"},max_completion_tokens:4096})}):await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":d,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:4096,system:m,messages:[{role:"user",content:g}],tools:[{type:"web_search_20250305",name:"web_search"}]})}),529===s.status)throw Error("API is overloaded — try again in a minute, or switch tier");if(429===s.status)throw Error("Rate limited — wait a moment and try again");if(!s.ok&&200!==s.status){let e=await s.text().catch(()=>"");throw Error(`API error ${s.status}: ${e.slice(0,200)||"Unknown error"}`)}let f=await s.json();if(f.error)throw Error(errStr(f.error));a=l?f.choices?.[0]?.message?.content||"":(f.content||[]).filter(e=>"text"===e.type).map(e=>e.text).join("");let _=a.replace(/```json\s*|```\s*/g,"").replace(/<\/?cite[^>]*>/g,"").replace(/<\/?antml:cite[^>]*>/g,"").trim();try{r=JSON.parse(_)}catch{let e=_.match(/\{[\s\S]*\}/);if(e)r=JSON.parse(e[0]);else throw Error("Could not parse response")}return t.status(200).json({success:!0,data:r})}async function handleRefine(e,t){let s,a,r;let{business_data:n,request:o,model:i}=e.body;if(!n||!o)return t.status(400).json({error:"business_data and request required"});let u="premium"===i||"premium-dark"===i,c=process.env.ANTHROPIC_API_KEY,l=process.env.OPENAI_API_KEY;if(!c&&!u)return t.status(500).json({error:"ANTHROPIC_API_KEY not set"});if(u&&!l)return t.status(500).json({error:"OPENAI_API_KEY not set — add it in Vercel env vars to use Premium ⚡"});let p=`You are editing a business website's content. The user will give you the current business JSON and a change request. Return ONLY valid JSON with the same schema, applying their requested changes. No markdown fences, no preamble, no explanation.
...
- Keep all values plain text. No HTML tags, no citations.`,d=`Current business data:
${JSON.stringify(n,null,2)}

Change request: ${o}

Return the updated JSON.`;if(s=u?await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${l}`},body:JSON.stringify({model:"gpt-5.5",messages:[{role:"system",content:p},{role:"user",content:d}],response_format:{type:"json_object"},max_completion_tokens:4096})}):await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":c,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:4096,system:p,messages:[{role:"user",content:d}]})}),529===s.status)throw Error("API is overloaded — try again in a minute");if(429===s.status)throw Error("Rate limited — wait a moment and try again");if(!s.ok){let e=await s.text().catch(()=>"");throw Error(`API error ${s.status}: ${e.slice(0,200)}`)}let h=await s.json();if(h.error)throw Error(errStr(h.error));a=u?h.choices?.[0]?.message?.content||"":(h.content||[]).filter(e=>"text"===e.type).map(e=>e.text).join("");let m=a.replace(/```json\s*|```\s*/g,"").replace(/<\/?cite[^>]*>/g,"").replace(/<\/?antml:cite[^>]*>/g,"").trim();try{r=JSON.parse(m)}catch{let e=m.match(/\{[\s\S]*\}/);if(e)r=JSON.parse(e[0]);else throw Error("Could not parse response")}return t.status(200).json({success:!0,data:r})}async function handlePhotos(e,t){let{name:s,address:a}=e.body;if(!s||!a)return t.status(400).json({error:"name and address required"});let r=process.env.GOOGLE_MAPS_API_KEY;if(!r)return t.status(200).json({success:!0,photos:[],message:"No Google Maps key — using stock photos"});let n=encodeURIComponent(`${s} ${a}`),o=await fetch(`https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${n}&inputtype=textquery&fields=place_id,name,photos&key=${r}`),i=await o.json();if(!i.candidates?.length)return t.status(200).json({success:!0,photos:[]});let u=i.candidates[0],c=await fetch(`https://maps.googleapis.com/maps/api/place/details/json?place_id=${u.place_id}&fields=photos&key=${r}`),l=await c.json(),p=(l.result?.photos||u.photos||[]).slice(0,12),d=[];for(let e of p){let t=`https://maps.googleapis.com/maps/api/place/photo?maxwidth=800&photo_reference=${e.photo_reference}&key=${r}`;try{let s=await fetch(t,{redirect:"manual"}),a=s.headers.get("location");d.push({url:a||t,width:e.width,height:e.height})}catch{d.push({url:t,width:e.width,height:e.height})}}return t.status(200).json({success:!0,place_id:u.place_id,photos:d})}r=(n.then?(await n)():n)[0],a()}catch(e){a(e)}})}};var t=require("../../webpack-api-runtime.js");t.C(e);var __webpack_exec__=e=>t(t.s=e),s=t.X(0,[222],()=>__webpack_exec__(5333));module.exports=s})();